    ...
    fluidRow(
      column(
        ...
        wellPanel(
          ...
          hr(),
          textInput(ns("palabra_clave"), "Palabra clave (ej. joyería, boutique):", "joyería, boutique"),
          numericInput(ns("radio"), "Radio de búsqueda (m):", 100, min = 100, step = 100),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    update_map <- function() {
      if (!is.null(user_location())) {
        ...
        token <- Sys.getenv(INEGI_API_KEY)
        meters <- input$radio
